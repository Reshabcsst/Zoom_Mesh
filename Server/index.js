...
let connectedUsers = [];
let rooms = [];
// create route to check if room exist
app.get('/api/room-exists/:roomId', (req, res) => {
    const { roomId } = req.params;
    const room = rooms.find(room => room.id === roomId);
    if (room) {
        // send responce that room esists
        ...
        // send responce that room does not exists
...
io.on('connection', (socket) => {
    ...
    socket.on('conn-init', data => {
        initializeConnectionHandeler(data, socket);
    });
});


const createNewRoom = (data, socket) => {
    console.log('Host is cretaing room');
    console.log(data);
    const { identity } = data;
    ...
    // push that user to the connectedUsers
    ...
    // emit an event to all users connected to that room about new users which are in this room 
...
const joinRoomHandler = (data, socket) => {
    ...
    // join as user which just is trying to join room passing room id
    const room = rooms.find(room => room.id === roomId);
    ...
    // emit to all users which are already in this room to prepare for peer connection
...
const disconnectHandler = (socket) => {
    ...
    if (user) {
        ...
        // TODO
        // close the room if amount of users which will stay in the room will be 0
...
const initializeConnectionHandeler = (data, socket) => {
